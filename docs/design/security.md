...
- **矛盾した結果は集合ごとピン留め禁止**: スキャン結果に `Changed`（既存ピンと不一致）または `Revoked`（`@revoked` 一致）が 1 つでもあれば、承認キー（`y`）は**何も追記しない**（`PinClass::poisons_result`）。`Changed` 行だけを書かない実装では不十分だった: OpenSSH は known_hosts の**いずれかの**行に一致した鍵を受理し、既存ピンのある鍵種を避けて別鍵種を提示できるため、`[new]` と表示された兄弟鍵を追記させるだけで正規のピンが警告なしに無力化される。UI もこの状態では「ピン留めは無効」と明示する。
...
- **ピン留め先は解決済み known_hosts ファイル**: 追記先は `ssh -G` が報告した `UserKnownHostsFile` の先頭（`keyscan_pin_target`）。`~/.ssh/known_hosts` 決め打ちは、カスタムファイル構成で「成功表示のまま ssh が読まない場所へ書く」無効動作になり、かつ意図的に隔離されたホストの信頼を既定ファイル利用の全エイリアスへ広げてしまう。
