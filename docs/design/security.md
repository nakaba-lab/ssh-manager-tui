...
- **既にピンのあるホストには何も追記しない（構造的規則）**: 承認キー（`y`）が追記できるのは、**この host に真正なピン（marker 無し・非パターン）が 1 つも無いとき**だけ（`PinBlocked::AlreadyPinned`）。加えて `Changed`／`Revoked` があれば同様に禁止（`PinBlocked::Contradicted`）。
  ...
  - **判定は追記直前に引き直した `matching_known_entries` に基づく**: モーダルを開いた時点のスナップショットで判断すると、開いている間に現れたピンの隣に追記できてしまう（OpenSSH は**署名検証の前に** known_hosts へ書くため、別端末での TOFU 承諾がこの窓に入る）。`y` は追記の直前に再読して `pin_block` をやり直す。
...
- **ピン留め先は解決済み known_hosts ファイル＋書き込み後の実効性検証**: 追記先は `ssh -G` が報告した `UserKnownHostsFile` の先頭（`keyscan_pin_target`→`primary_known_hosts_file`）。`~/.ssh/known_hosts` 決め打ちは、カスタムファイル構成で「成功表示のまま ssh が読まない場所へ書く」無効動作になり、かつ意図的に隔離されたホストの信頼を既定ファイル利用の全エイリアスへ広げてしまう。
  ...
  - **`UserKnownHostsFile none` を拒否する**: OpenSSH は `none` を「ユーザーファイルを読まない」の標識として扱い、`ssh -G` もそのまま出す。これをファイル名と解釈すると CWD にゴミを作って成功と表示し、しかも `is_host_known` が true になってオートフィルまで武装してしまう。**判定はリスト全体に対して行う**（`files == ["none"]`）: OpenSSH は `none` を単独でしか受け付けないため、長いリストの中の `none` は標識ではなく `…/my none dir/…` のようなパス成分であり、要素単位で落とすとパスが分断されて真正ピンを見落とす。
  ...
  - **最後は OpenSSH に確認させる**: 追記後に、**いま書いた鍵（`key_type` + `key_b64`）が**OpenSSH のマッチャから見えるかを確認し、見えなければ成功と報告せず書いた場所を添えて警告する（「ピンが 1 つでも見えるか」では既存ピンが条件を満たしてしまう）。**参照するファイル集合は追記の前に一度だけ解決して固定する**: 解決は `exists()` を手がかりにするため、追記後に解決し直すと**書き込み自身が作った迷子ファイル**が集合に入り込み、検証が自己成就してしまう。
