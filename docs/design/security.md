...
- **ピン留め先は解決済み known_hosts ファイル＋書き込み後の実効性検証**: 追記先は `ssh -G` が報告した `UserKnownHostsFile` の先頭（`keyscan_pin_target`→`primary_known_hosts_file`）。`~/.ssh/known_hosts` 決め打ちは、カスタムファイル構成で「成功表示のまま ssh が読まない場所へ書く」無効動作になり、かつ意図的に隔離されたホストの信頼を既定ファイル利用の全エイリアスへ広げてしまう。
  ...
  - **曖昧な一覧ではスキャンしない**: `"<fileA> <fileB>"` という名前のファイルが実在すると、報告された語の並びは「別々のパス」とも「1 つの空白入りパス」とも読める。coalesce は貪欲に長い方を採るため、`fileB` 以降のピンが見えなくなる一方で書き込み先は ssh が実際に読むファイルのまま、という組み合わせが成立した（#46 round 11・実証済み）。読み取り側は貪欲な推測でも「エントリを取りこぼす」だけだが、書き込み側は許されないので拒否する（`known_hosts_paths_are_ambiguous`）。判定の形は **3 度**間違えているので明記する:
    ...
    - **連結単位のローカルな規則は 3 度とも間違えた**（rounds 12・13・14）。この履歴が「直接問う」形に至った理由なので残す。
  ...
  - **最後は OpenSSH に確認させる**: 追記後に、**いま書いた鍵（`key_type` + `key_b64`）が**OpenSSH のマッチャから見えるかを確認し、見えなければ成功と報告せず書いた場所を添えて警告する（「ピンが 1 つでも見えるか」では既存ピンが条件を満たしてしまう）。解決は追記の**後**に行う: 初回ピンではファイルがまだ存在せず、事前解決だと読み取り集合から落ちて偽の失敗になるため。
    - **この検証は書き込み先そのものの誤りは見抜けない（受け入れた残存リスク・#46 round 15 で実証）**: 再読は書き込み先を選んだのと**同じ貪欲な連結**を通るため、書き込み先自体が誤った推測だった場合は、その誤った場所を読んで「見えている」と答える。具体的には、**ssh の 1 つ目の known_hosts ファイルがまだ存在せず**、その置き場に `"<fileA> <fileB>"` という名前の通常ファイルが先に置かれていると、書き込みはそのファイルに落ち、成功と表示され、`is_host_known` まで true になる（OpenSSH は何のピンも持たないまま）。
      - **権限昇格ではない**: この形を作れる攻撃者は `<fileA>` 自体を作れる＝OpenSSH がそのまま信頼する鍵を置けるので、sshm を介さないより強い攻撃が既に成立している。塞ぐべきは「守れていない保証を主張していた記述」であって書き込み経路ではない、と判断した。
      - **塞げない理由**: `ssh -G` の出力とファイルシステムだけでは、この形と**正当な初回ピン**（`C:\Users\First Last\.ssh\known_hosts` がまだ無い＝Windows-first の主経路）を区別できない。「複数語の書き込み先で、まだ存在しないものは拒否する」も「より短い接頭辞も書き込み可能なら拒否する」も、正当な側を殺す。
      - **緩和（限定的）**: モーダルは追記前に**実際の書き込み先パスをそのまま表示する**（`pin_destination`）ので、`"<fileA> <fileB>"` のような不自然な連結先は `y` を押す前に目視できる。ただしこの widget は折り返さず**クリップする**（`ui.md` の当該項）ため、連結先を怪しいと判じる材料である行末は、深いパス＋狭い端末では切り落とされる。80 桁程度あれば見える、という限定付きの緩和である。
