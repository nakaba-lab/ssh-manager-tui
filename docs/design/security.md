...
- **`Match exec` があるホストはスキャンしない／見通せない config でもスキャンしない**: `ssh -G` は述語を**実行する**。接続経路・SFTP 経路が同じ理由で解決を避けているのに、メニュー項目を開くだけで副作用を起こすのは筋が通らない。加えて非決定的な述語は、スキャン時の解決と実接続の解決を食い違わせ、読むピンと書くファイルが別物になりうる。
  - **検査対象は `ssh -G` が読むファイル**。`app.config` は `--config` で別ファイルを指しうるので、`~/.ssh/config` と読み込み済み config の**両方**を見る。
  - **`Include` があれば拒否する**: 取り込み先を解析しないため `Match exec` を見通せない。実際、included ファイル内の `Match exec` はガードを素通りして述語が実行された（#46 round 9）。ここでも「不完全な材料で判断しない」を適用する。
