...
- **`Match exec` があるホストはスキャンしない／見通せない config でもスキャンしない**: `ssh -G` は述語を**実行する**。接続経路・SFTP 経路が同じ理由で解決を避けているのに、メニュー項目を開くだけで副作用を起こすのは筋が通らない。加えて非決定的な述語は、スキャン時の解決と実接続の解決を食い違わせ、読むピンと書くファイルが別物になりうる。
  - **検査対象は `ssh -G` が読むファイル全部**: `~/.ssh/config`・`/etc/ssh/ssh_config`・それらが `Include` するもの、加えて読み込み済み config（`--config` で別ファイルを指しうる）。読み込み済みファイルだけを見る実装では、included ファイル内の `Match exec` が素通りして述語が実行された（#46 round 9）。
  - **`Include` は展開して見る**（`read_config_with_includes`。glob は最終要素のみ・深さ 16 まで）。単に「`Include` があれば拒否」にすると、`Include ~/.ssh/conf.d/*` が一般的なため実運用の大半で機能が死ぬ。**展開できない形**（ディレクトリ要素の glob・読めない取り込み先）は `None` を返して拒否する＝「不完全な材料で判断しない」。
  - **バイト列として読む**: ssh はバイトを読むので、`read_to_string` は非 UTF-8 のバイトが 1 つあるだけで失敗する。それを `unwrap_or_default()` で空文字に落としていたため、コメント中の Latin-1 バイト 1 つでガード全体が無言で無効化されていた（#46 round 10）。
  - **`Match !exec` も対象**: 否定は評価を伴うので OpenSSH は述語を実行する。装飾を外してから比較しないと素通りする（#46 round 10）。
...
- **ピン留め先は解決済み known_hosts ファイル＋書き込み後の実効性検証**: 追記先は `ssh -G` が報告した `UserKnownHostsFile` の先頭（`keyscan_pin_target`→`primary_known_hosts_file`）。`~/.ssh/known_hosts` 決め打ちは、カスタムファイル構成で「成功表示のまま ssh が読まない場所へ書く」無効動作になり、かつ意図的に隔離されたホストの信頼を既定ファイル利用の全エイリアスへ広げてしまう。
  ...
  - **残る既知の限界（デコイ）**: 読み取り側の coalesce・書き込み先の決定・追記後の検証はいずれもファイルの存在を手がかりにするため、`"<fileA> <fileB>"` という名前のファイルを作れる者は、`fileB` のピンを隠したまま `fileA` へ書かせられる。必要な権限は**一覧の先頭要素があるディレクトリへの書き込み**（当初「隠す対象のディレクトリ」と書いていたのは誤り）。ただしそのディレクトリは sshm が書き込む先そのものなので、同じ権限があれば known_hosts を直接編集でき、権限昇格にはならない。オプションをまたぐ形（global のピンを user の `.ssh` から隠す）は上記の「オプションごとに解決する」で塞いである。
