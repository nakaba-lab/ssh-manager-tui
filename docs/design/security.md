...
- **`ssh-keygen -F` が見られない信頼経路があるならスキャンしない**: `KnownHostsCommand`（OpenSSH 8.5+・プログラムが known_hosts 行を供給する）と `VerifyHostKeyDNS yes`（DNSSEC 署名付き SSHFP が known_hosts 無しでホストを認証する）は、いずれもファイルを読むだけでは見えない。ここで「一致行が無い＝未ピン」と解釈すると、確立済みの信頼経路の隣に鍵を足すことになる（`KnownHostsCommand` で実証済み）。`has_external_trust_source` を立ててスキャン自体を拒否する。
- **`Match exec` があるホストはスキャンしない**: `ssh -G` は述語を**実行する**。接続経路・SFTP 経路が同じ理由で解決を避けているのに、メニュー項目を開くだけで副作用を起こすのは筋が通らない。加えて非決定的な述語は、スキャン時の解決と実接続の解決を食い違わせ、読むピンと書くファイルが別物になりうる。加えて `Changed`／`Revoked` があれば同様に禁止（`PinBlocked::Contradicted`）。
...
- **ピン留め先は解決済み known_hosts ファイル＋書き込み後の実効性検証**: 追記先は `ssh -G` が報告した `UserKnownHostsFile` の先頭（`keyscan_pin_target`→`primary_known_hosts_file`）。`~/.ssh/known_hosts` 決め打ちは、カスタムファイル構成で「成功表示のまま ssh が読まない場所へ書く」無効動作になり、かつ意図的に隔離されたホストの信頼を既定ファイル利用の全エイリアスへ広げてしまう。
  ...
  - **残る既知の限界（デコイ）**: 読み取り側の coalesce・書き込み先の決定・追記後の検証はいずれもファイルの存在を手がかりにするため、`"<fileA> <fileB>"` という名前のファイルを作れる者は、`fileB` のピンを隠したまま `fileA` へ書かせられる。ただしこれには**隠したいファイルのあるディレクトリへの書き込み権限**が要り、その権限があれば known_hosts を直接編集できるので、権限昇格にはならない。
