...
- **ピン留め先は解決済み known_hosts ファイル＋書き込み後の実効性検証**: 追記先は `ssh -G` が報告した `UserKnownHostsFile` の先頭（`keyscan_pin_target`→`primary_known_hosts_file`）。`~/.ssh/known_hosts` 決め打ちは、カスタムファイル構成で「成功表示のまま ssh が読まない場所へ書く」無効動作になり、かつ意図的に隔離されたホストの信頼を既定ファイル利用の全エイリアスへ広げてしまう。
  ...
  - **復元できないパスへは書かない**: `ssh -G` は空白の連続を 1 つに潰すため、`…/a  b/known_hosts` のようなパスは**原理的に復元不能**。この場合 coalesce は単語単位に退化し、先頭要素は切り詰められた接頭辞になる。そこへ書くと OpenSSH が読まない迷子ファイルができ、しかも**その接頭辞自身が読み取り集合の要素なので事後検証も自己成就して成功と報告する**。したがって `primary_known_hosts_file` は、coalesce 後の**全要素**が書き込み可能に見えないかぎり `None` を返す（＝スキャンを拒否する）。
