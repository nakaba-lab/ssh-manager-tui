...
- **ピン留め先は解決済み known_hosts ファイル＋書き込み後の実効性検証**: 追記先は `ssh -G` が報告した `UserKnownHostsFile` の先頭（`keyscan_pin_target`→`primary_known_hosts_file`）。`~/.ssh/known_hosts` 決め打ちは、カスタムファイル構成で「成功表示のまま ssh が読まない場所へ書く」無効動作になり、かつ意図的に隔離されたホストの信頼を既定ファイル利用の全エイリアスへ広げてしまう。
  ...
  - **`UserKnownHostsFile none` を拒否する**: OpenSSH は `none` を「ユーザーファイルを読まない」の標識として扱い、`ssh -G` もそのまま出す。これをファイル名と解釈すると CWD にゴミを作って成功と表示し、しかも `is_host_known` が true になってオートフィルまで武装してしまう。解決結果が `none` だけならピン留めを拒否する。
  - **最後は OpenSSH に確認させる**: 追記後に `matching_known_entries` を再実行し、**いま書いた鍵（`key_type` + `key_b64`）が**OpenSSH のマッチャから見えなければ成功と報告せず、書いた場所を添えて警告する（「ピンが 1 つでも見えるか」では既存ピンが条件を満たしてしまう）。「読まれない場所に書いて成功と表示する」失敗はこの機能が繰り返し踏んだ経路なので、経路ごとの推論ではなく事後検証で塞ぐ。
  - **残る既知のギャップ**: 検証は読み取り側と同じファイル一覧の復元を通るため、復元自体が外す場合（`coalesce_existing_paths` が扱えない**連続空白/タブを含むパス**）は検出できない。これは `resolve.rs` に既知の fail-safe ギャップとして記録されているものと同根で、読み取り側では「ホストが未知扱いになる」＝安全側に倒れるが、書き込み側では検出漏れになる。
