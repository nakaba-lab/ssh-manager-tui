...
- **既存ピンの「確認」を条件にする（#46）**: ピン留めできるのは、既存ピンの鍵種がすべてスキャンで確認され（`unconfirmed_pin_types` が空）、かつ `Changed`／`Revoked` が無い場合だけ。`Changed` の検出だけでは、攻撃者がピン済み鍵種を提示しない全 `New` の結果で素通りする（判断根拠は `docs/design/security.md`）。
