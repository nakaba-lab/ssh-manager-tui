...
- **単体（TDD）**: Red-Green-Refactor。`config/`・`os/` のファイル内に inline `#[cfg(test)]`（現在 260 テスト）。回帰テストはコメントでバグ id（B1・B5 等）を付けて保存する。純粋（実行時に `ssh`/`ssh-keygen`/`sftp`/`wt` を spawn しない）でヘッドレス完走。
